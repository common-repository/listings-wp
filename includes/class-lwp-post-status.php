...
class Listings_Wp_Post_Status {
	...
	function archive_post_status(){
	    register_post_status( 'archive', array(
			...
			'label_count'               => _n_noop( 'Archive <span class="count">(%s)</span>', 'Archived <span class="count">(%s)</span>', 'listings-wp' ),
		) );
	}
	...
	function post_screen_js(){
	    ...
	    if ( 'draft' !== $post->post_status && 'pending' !== $post->post_status ) {
			...
			jQuery( document ).ready( function( $ ) {
				$( '#post_status' ).append( '<option value="archive"><?php esc_html_e( 'Archived', 'listings-wp' ) ?></option>' );
			} );
			</script>
			<?php
	...
	function edit_screen_js() {
		...
		jQuery( document ).ready( function( $ ) {
			...
			$( 'select[name="_status"]' ).append( '<option value="archive"><?php esc_html_e( 'Archived', 'listings-wp' ) ?></option>' );
...
return new Listings_Wp_Post_Status();