 ...
 * Get an agents ID
 */
function listings_wp_agent_ID() {

	$id = listings_wp_meta( 'agent' );
	if( $id ) {
		return $id;
	} else {
	...
	return listings_wp_shortcode_att( 'id', 'listings_wp_agent' );
 ...
 */
function listings_wp_agent_listings_count( $agent_id ) {
 ...
 * @return  array
 */
function listings_wp_get_allowed_roles( $user ) {
    $allowed = array();

    if ( in_array( 'listings_wp_agent', $user->roles ) ) {
        $allowed[] = 'listings_wp_agent';
    } 
 ...
 */
function listings_wp_editable_roles( $roles ) {
    $user = wp_get_current_user();
    if ( in_array( 'listings_wp_agent', $user->roles ) ) {
        $allowed = listings_wp_get_allowed_roles( $user );
...
add_filter( 'editable_roles', 'listings_wp_editable_roles' );

/**
 * Prevent users deleting/editing users with a role outside their allowance.
 */
function listings_wp_map_meta_cap( $caps, $cap, $user_ID, $args ) {
    if ( ( $cap === 'edit_user' || $cap === 'delete_user' ) && $args ) {
        $the_user = get_userdata( $user_ID ); // The user performing the task
        $user     = get_userdata( $args[0] ); // The user being edited/deleted

        // only run for agents
        if ( in_array( 'listings_wp_agent', $the_user->roles ) ) {
	        if ( $the_user && $user && $the_user->ID != $user->ID /* User can always edit self */ ) {
	            $allowed = listings_wp_get_allowed_roles( $the_user );
...
add_filter( 'map_meta_cap', 'listings_wp_map_meta_cap', 10, 4 );
 ...
 */
function get_listings_wp_img_meta( $user_id, $size = 'thumbnail' ) {
    global $post;
    ...
    $attachment_upload_url = esc_url( get_the_author_meta( 'listings_wp_upload_meta', $user_id ) );
    ...
    $attachment_ext_url = esc_url( get_the_author_meta( 'listings_wp_meta', $user_id ) );
 ...
 * @return string
 */
function listings_wp_filter_avatar( $avatar, $identifier, $size, $alt ) {
    if ( $user = listings_wp_get_user_by_id_or_email( $identifier ) ) {
        if ( $custom_avatar = get_listings_wp_img_meta( $user->ID, 'thumbnail' ) ) {
...
add_filter( 'get_avatar', 'listings_wp_filter_avatar', 1, 5 );
 ...
 * @return WP_User
 */
function listings_wp_get_user_by_id_or_email( $identifier ) {
